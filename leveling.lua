...
local function startLeveling(tog)
	...
	LV_Thread = task.spawn(function()
		...
		for idx, targetUUID in ipairs(snapshot) do
			...
			lvAddLog(string.format("[%d/%d] %s — Lv%d → 100", idx, total, petName, ageNow), T.ACCENT)
			lvSetStatus(string.format("[%d/%d] %s", idx, total, petName), T.TEXT)

			if ageNow >= 100 then
				lvAddLog(string.format("✓ Already Lv100, skip", petName), T.DIM)
				local i = table.find(D.leveling.targets, targetUUID); if i then table.remove(D.leveling.targets, i); saveD() end
			...
			while LV_Running do
				...
				lvSetStatus(string.format("Lv%d/100 | %s", age2, petName), T.DIM)
				...
				if age2 >= lastLog + 10 then
					lvAddLog(string.format("  Lv%d/100  %s", age2, petName), T.DIM)
				...
				if age2 >= 100 then
					...
					lvAddLog(string.format("✓ DONE  %s  Lv100  (%s)", petName, UI.fmtTime(elapsed)), T.SUCCESS)
